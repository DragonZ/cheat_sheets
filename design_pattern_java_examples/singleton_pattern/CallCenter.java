...
/* Because the tutorial example is not really good, for singleton pattern
 * I use cracking the coding interview, object-oriented design (chapter 8) question 2 as an example
 * basically the calling center is the singleton class
 * the most important structure is Call, Employee and the CallCenter
 * I have attached the UML diagram to help to understand 
 * 
 * THANKS TO THE ORIGINAL SOLUTION AUTHOR, I just modify a little bit


 * CallCenter is implement as a singleton class. It represents the body of the program,
 * and all calls are funneled first through it. 


build CallCenter first:
construct and initialize employees with their corresponding ranks(levels)-> CallCenter(){}
get the instance->getInstance();
get the first avialble person, if no one is free, dispatch the call->getHandlerforcall(Call), dispatch(Call)
assign a pending waiting call to the free person->assignNewCall(employ)
*/
public class CallCenter {
    ...
    /* Initialize with 10 respondents, 1 manager, and 1 director. */
